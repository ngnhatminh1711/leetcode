...
/* ================= 1. ARRAY BASICS ================= */
{
  ...
  examples: [
    ...
    { name: "Product of Array Except Self", difficulty: "Medium", note: "Tính mảng kết quả mà mỗi phần tử là tích của TẤT CẢ phần tử khác (trừ chính nó), không dùng phép chia.", hint: "Duyệt 2 lượt: lượt 1 từ trái sang phải, tính tích các phần tử BÊN TRÁI mỗi vị trí. Lượt 2 từ phải sang trái, nhân thêm tích các phần tử BÊN PHẢI. Kết quả tại mỗi vị trí là tích của 2 lượt đó." },
    { name: "Find All Numbers Disappeared in an Array", difficulty: "Easy", note: "Tìm các số từ 1 đến n bị thiếu trong mảng có độ dài n (giá trị trong khoảng 1..n), không dùng thêm bộ nhớ phụ.", hint: "Duyệt mảng, với mỗi giá trị v, đánh dấu vị trí (v-1) bằng cách biến giá trị tại đó thành âm (nếu chưa âm). Sau khi duyệt xong, vị trí nào còn dương nghĩa là số (index+1) đó bị thiếu." },
    { name: "Rotate Array", difficulty: "Medium", note: "Xoay mảng sang phải k bước tại chỗ (in-place), dùng kỹ thuật đảo ngược 3 lần thay vì tạo mảng mới.", hint: "Xoay phải k bước tương đương với: đảo ngược TOÀN BỘ mảng, rồi đảo ngược riêng k phần tử đầu, rồi đảo ngược riêng phần còn lại. Vì sao 3 bước đảo ngược này lại cho kết quả đúng?" }
  ],
  walkthrough: {
...
public class FrequencyCounterTemplate {
  ...
  examples: [
    ...
    { name: "Contains Duplicate II", difficulty: "Easy", note: "Kiểm tra có 2 phần tử trùng giá trị mà chỉ số cách nhau không quá k hay không, dùng Map lưu index gần nhất.", hint: "Duyệt mảng, lưu vào map {giá trị: index gần nhất đã thấy}. Với mỗi phần tử, nếu giá trị đã có trong map VÀ (index hiện tại - index đã lưu) <= k, trả về true ngay." },
    { name: "Isomorphic Strings", difficulty: "Easy", note: "Kiểm tra 2 chuỗi có thể ánh xạ ký tự 1-1 với nhau hay không (isomorphic), dùng 2 Map để đảm bảo ánh xạ 2 chiều nhất quán.", hint: "Cần 2 map: 1 map ánh xạ ký tự chuỗi s -> ký tự chuỗi t, 1 map ánh xạ ngược lại t -> s. Duyệt đồng thời 2 chuỗi, kiểm tra ánh xạ ở cả 2 chiều luôn nhất quán — vì sao chỉ 1 chiều là chưa đủ?" },
    { name: "First Unique Character in a String", difficulty: "Easy", note: "Tìm chỉ số của ký tự không lặp lại đầu tiên trong chuỗi, dùng frequency counter.", hint: "Duyệt chuỗi 1 lần để đếm tần suất từng ký tự vào map. Duyệt chuỗi lần 2 theo đúng thứ tự ban đầu, trả về chỉ số đầu tiên có tần suất trong map bằng 1." }
  ],
  walkthrough: {
    problemName: "Two Sum",
    difficulty: "Easy",
...
/* ================= 3. TWO POINTERS ================= */
{
  ...
  examples: [
    ...
    { name: "Squares of a Sorted Array", difficulty: "Easy", note: "Cho mảng đã sắp xếp (có thể chứa số âm), trả về mảng bình phương của các phần tử theo thứ tự tăng dần.", hint: "Bình phương lớn nhất luôn nằm ở 1 trong 2 đầu mảng (số âm nhỏ nhất hoặc số dương lớn nhất). Dùng 2 con trỏ từ 2 đầu, so sánh bình phương, điền kết quả từ CUỐI mảng kết quả về đầu." },
    { name: "Backspace String Compare", difficulty: "Easy", note: "So sánh 2 chuỗi có ký tự '#' (biểu thị phím xóa lùi) sau khi áp dụng xóa, dùng Two Pointers duyệt từ CUỐI chuỗi.", hint: "Duyệt từ cuối mỗi chuỗi về đầu. Gặp '#' thì tăng biến đếm 'cần xóa' và bỏ qua ký tự tiếp theo; gặp ký tự thường mà biến đếm > 0 thì cũng bỏ qua (bị xóa). So sánh ký tự 'sống sót' của 2 chuỗi tại mỗi bước." },
    { name: "Two Sum IV - Input is a BST", difficulty: "Easy", note: "Kiểm tra có 2 node trong Binary Search Tree có tổng bằng target hay không, kết hợp duyệt in-order (cho dãy tăng dần) với Two Pointers.", hint: "Duyệt in-order BST để lấy ra mảng giá trị đã SẮP XẾP TĂNG DẦN. Sau đó áp dụng Two Pointers y hệt bài Two Sum II trên mảng đó." }
  ],
  walkthrough: {
...
public class SlidingWindowTemplate {
  ...
  examples: [
    ...
    { name: "Find All Anagrams in a String", difficulty: "Medium", note: "Tìm mọi vị trí bắt đầu của các đoạn con là anagram của chuỗi p, dùng cửa sổ kích thước cố định + frequency counter.", hint: "Tạo frequency counter của p. Trượt cửa sổ có độ dài bằng p qua chuỗi s, so sánh frequency counter của cửa sổ hiện tại với frequency counter của p — nếu khớp, ghi nhận vị trí bắt đầu." },
    { name: "Max Consecutive Ones III", difficulty: "Medium", note: "Tìm đoạn con dài nhất chỉ toàn số 1 sau khi được phép đổi tối đa k số 0 thành 1.", hint: "Trong cửa sổ, đếm số lượng số 0. Khi số lượng số 0 vượt quá k, thu hẹp left cho đến khi số 0 trong cửa sổ <= k. Độ dài cửa sổ lớn nhất từng đạt được chính là đáp án." },
    { name: "Minimum Window Substring", difficulty: "Medium", note: "Tìm đoạn con ngắn nhất của s chứa toàn bộ ký tự của t (kể cả trùng lặp), dùng Sliding Window với 2 frequency counter.", hint: "Dùng map đếm ký tự cần có (từ t) và map đếm ký tự đang có (trong cửa sổ). Mở rộng right cho đến khi cửa sổ chứa đủ mọi ký tự cần thiết, sau đó thử thu hẹp left để tìm cửa sổ ngắn nhất vẫn còn đủ." }
  ],
  walkthrough: {
...
public class StackTemplate {
  ...
  examples: [
    ...
    { name: "Next Greater Element I", difficulty: "Easy", note: "Tìm phần tử lớn hơn gần nhất bên phải cho mỗi phần tử trong mảng con, dùng Stack lưu chỉ số/giá trị chưa tìm được đáp án.", hint: "Duyệt mảng lớn, dùng stack lưu các phần tử CHƯA tìm được 'phần tử lớn hơn tiếp theo'. Khi gặp giá trị mới lớn hơn đỉnh stack, đó chính là đáp án cho phần tử ở đỉnh — pop ra và ghi nhận." },
    { name: "Decode String", difficulty: "Medium", note: "Giải mã chuỗi dạng k[encoded_string] (ví dụ '3[a2[c]]' thành 'accaccacc'), dùng Stack để xử lý lồng nhau.", hint: "Dùng 2 stack (hoặc 1 stack lưu cặp số lần lặp + chuỗi hiện tại): khi gặp '[', đẩy số lần lặp và chuỗi đang xây vào stack, reset lại để xây chuỗi mới. Khi gặp ']', pop ra, lặp chuỗi hiện tại theo số đã lưu rồi nối vào chuỗi trước đó." },
    { name: "Simplify Path", difficulty: "Medium", note: "Rút gọn đường dẫn kiểu Unix (xử lý '.', '..', '//'), dùng Stack để mô phỏng việc đi vào/lùi ra thư mục.", hint: "Tách đường dẫn theo dấu '/'. Với mỗi phần: nếu là '..' thì pop khỏi stack (lùi ra thư mục cha, nếu có); nếu là '.' hoặc rỗng thì bỏ qua; ngược lại push vào stack (đi vào thư mục con)." }
  ],
  walkthrough: {
...
/* ================= 6. BINARY SEARCH ================= */
{
  ...
  examples: [
    ...
    { name: "Find First and Last Position of Element in Sorted Array", difficulty: "Medium", note: "Tìm vị trí đầu tiên và cuối cùng của target trong mảng đã sắp xếp, dùng 2 lần Binary Search.", hint: "Chạy Binary Search 2 lần với biến thể khác nhau: lần 1 tìm vị trí NHỎ NHẤT thỏa nums[i] == target (khi tìm thấy, tiếp tục thu hẹp về bên trái thay vì dừng ngay), lần 2 tương tự nhưng thu hẹp về bên phải để tìm vị trí LỚN NHẤT." },
    { name: "Sqrt(x)", difficulty: "Easy", note: "Tính căn bậc hai của x, lấy phần nguyên, không dùng hàm sqrt có sẵn — Binary Search trên khoảng giá trị kết quả.", hint: "Tìm kiếm trên khoảng [0, x]: với mỗi giá trị thử mid, kiểm tra mid*mid có <= x không. Đây là hàm kiểm tra đơn điệu (đúng rồi sai theo 1 chiều), phù hợp Binary Search để tìm giá trị lớn nhất thỏa điều kiện." },
    { name: "Capacity To Ship Packages Within D Days", difficulty: "Medium", note: "Tìm sức chứa nhỏ nhất của tàu để chở hết hàng trong D ngày, Binary Search trên KẾT QUẢ giống bài Koko Eating Bananas.", hint: "Tìm kiếm trên khoảng sức chứa có thể [max(weights), sum(weights)]. Với mỗi sức chứa thử nghiệm, mô phỏng xem cần bao nhiêu ngày để chở hết — hàm này đơn điệu theo sức chứa, phù hợp Binary Search." }
  ],
  walkthrough: {
...
public class LinkedListTemplate {
  ...
  examples: [
    ...
    { name: "Intersection of Two Linked Lists", difficulty: "Easy", note: "Tìm node giao nhau của 2 danh sách liên kết (nếu có), dùng kỹ thuật 2 con trỏ đi qua cả 2 danh sách.", hint: "Cho 2 con trỏ, mỗi con trỏ đi hết danh sách của mình rồi chuyển sang đi tiếp danh sách còn lại. Nếu 2 danh sách có giao nhau, 2 con trỏ sẽ gặp nhau đúng tại điểm giao sau khi đi qua tổng độ dài bằng nhau — vì sao?" },
    { name: "Odd Even Linked List", difficulty: "Medium", note: "Nhóm các node ở vị trí lẻ lại với nhau, rồi đến các node ở vị trí chẵn, giữ nguyên thứ tự tương đối, làm tại chỗ.", hint: "Dùng 2 con trỏ tách riêng: 1 con trỏ nối các node lẻ với nhau, 1 con trỏ nối các node chẵn với nhau. Cuối cùng, nối đuôi danh sách lẻ với đầu danh sách chẵn." },
    { name: "Copy List with Random Pointer", difficulty: "Medium", note: "Sao chép sâu (deep copy) một danh sách liên kết có thêm con trỏ 'random' trỏ tới node bất kỳ, dùng Hash Map ánh xạ node cũ -> node mới.", hint: "Duyệt lần 1: tạo node mới cho từng node cũ, lưu ánh xạ {node cũ: node mới} vào map. Duyệt lần 2: với mỗi node mới, gán next và random bằng cách tra map từ node cũ tương ứng." }
  ],
  walkthrough: {
...
public class TreeTemplate {
  ...
  examples: [
    ...
    { name: "Validate Binary Search Tree", difficulty: "Medium", note: "Kiểm tra một cây nhị phân có đúng là Binary Search Tree hợp lệ không, dùng DFS truyền theo khoảng giá trị cho phép.", hint: "Chỉ so sánh với node cha là chưa đủ — cần truyền xuống 1 khoảng (min, max) hợp lệ cho mỗi node khi đệ quy. Node bên trái phải nhỏ hơn giá trị cha VÀ vẫn nằm trong khoảng đã truyền từ tổ tiên xa hơn." },
    { name: "Kth Smallest Element in a BST", difficulty: "Medium", note: "Tìm phần tử nhỏ thứ k trong Binary Search Tree, dùng tính chất duyệt in-order của BST luôn cho dãy tăng dần.", hint: "Duyệt in-order (trái -> gốc -> phải) của BST luôn cho ra dãy giá trị TĂNG DẦN. Duyệt theo thứ tự đó và đếm số phần tử đã duyệt qua — dừng lại khi đếm đến k." },
    { name: "Binary Tree Right Side View", difficulty: "Medium", note: "Liệt kê giá trị các node nhìn thấy được khi đứng ở bên phải cây, dùng BFS và lấy node cuối cùng mỗi tầng.", hint: "Dùng BFS duyệt theo từng tầng. Tại mỗi tầng, giá trị nhìn thấy từ bên phải chính là node CUỐI CÙNG được xử lý trong tầng đó." }
  ],
  walkthrough: {
...
public class BacktrackingTemplate {
  ...
  examples: [
    ...
    { name: "Combinations", difficulty: "Medium", note: "Liệt kê mọi tổ hợp chọn k phần tử từ n số nguyên đầu tiên (1..n), dùng Backtracking cơ bản.", hint: "Giống Subsets nhưng chỉ giữ lại các tập con có đúng k phần tử. Có thể cắt tỉa sớm: nếu số phần tử còn lại không đủ để đạt k, dừng nhánh đó ngay để tăng tốc." },
    { name: "Generate Parentheses", difficulty: "Medium", note: "Sinh ra mọi tổ hợp dấu ngoặc hợp lệ với n cặp ngoặc, dùng Backtracking với điều kiện ràng buộc số ngoặc mở/đóng.", hint: "Tại mỗi bước, có 2 lựa chọn: thêm '(' (chỉ khi số dấu mở đã dùng < n) hoặc thêm ')' (chỉ khi số dấu đóng đã dùng < số dấu mở đã dùng, để đảm bảo hợp lệ). Base case: khi độ dài path = 2n." },
    { name: "Combination Sum II", difficulty: "Medium", note: "Tổ hợp có tổng bằng target từ mảng CÓ chứa phần tử trùng lặp, mỗi phần tử chỉ dùng 1 lần, không sinh tổ hợp trùng.", hint: "Sắp xếp mảng trước. Khác Combination Sum, mỗi phần tử chỉ dùng 1 lần nên đệ quy tiếp với i+1 (không phải i). Để tránh trùng: trong vòng lặp, bỏ qua phần tử nếu nó giống phần tử liền trước NÓ ở cùng cấp đệ quy." }
  ],
  walkthrough: {
    problemName: "Subsets",
    difficulty: "Medium",
...
public class DpTemplate {
  ...
  examples: [
    ...
    { name: "Maximum Product Subarray", difficulty: "Medium", note: "Tìm dãy con liên tiếp có tích lớn nhất — khác Maximum Subarray vì số âm nhân số âm có thể ra số dương lớn.", hint: "Cần theo dõi CẢ 2 giá trị tại mỗi vị trí: tích lớn nhất và tích nhỏ nhất kết thúc tại đó. Vì sao? Vì nếu gặp số âm, tích nhỏ nhất (có thể đang rất âm) nhân với số âm mới có thể trở thành tích lớn nhất." },
    { name: "Partition Equal Subset Sum", difficulty: "Medium", note: "Kiểm tra mảng có thể chia thành 2 tập con có tổng bằng nhau hay không — biến thể của bài toán cái túi (0/1 Knapsack).", hint: "Nếu tổng cả mảng là số lẻ, chắc chắn không chia được. Nếu chẵn, bài toán trở thành: có tồn tại tập con nào có tổng đúng bằng (tổng/2) không? Đây là bài toán Knapsack cổ điển — dp[s] = true nếu có tập con tổng bằng s." },
    { name: "Longest Common Subsequence", difficulty: "Medium", note: "Tìm độ dài dãy con chung dài nhất giữa 2 chuỗi (không cần liên tiếp), dùng DP 2 chiều.", hint: "dp[i][j] là độ dài LCS của 2 chuỗi con text1[0..i] và text2[0..j]. Nếu ký tự tại i và j giống nhau, dp[i][j] = dp[i-1][j-1] + 1. Nếu khác, dp[i][j] = max(dp[i-1][j], dp[i][j-1])." }
  ],
  walkthrough: {
...
public class GreedyTemplate {
  ...
  examples: [
    ...
    { name: "Merge Intervals", difficulty: "Medium", note: "Hợp nhất các khoảng chồng lấn nhau thành các khoảng lớn hơn, sắp xếp theo điểm bắt đầu rồi tham lam gộp dần.", hint: "Sắp xếp các khoảng theo điểm BẮT ĐẦU tăng dần. Duyệt qua từng khoảng: nếu điểm bắt đầu của khoảng hiện tại <= điểm kết thúc của khoảng đang gộp, mở rộng khoảng đang gộp; nếu không, khoảng đang gộp đã hoàn tất, bắt đầu khoảng mới." },
    { name: "Minimum Number of Arrows to Burst Balloons", difficulty: "Medium", note: "Tìm số mũi tên tối thiểu để bắn vỡ hết các bong bóng (biểu diễn bằng khoảng), sắp xếp theo điểm kết thúc và bắn tham lam.", hint: "Sắp xếp các khoảng theo điểm KẾT THÚC tăng dần. Bắn mũi tên đầu tiên tại điểm kết thúc của khoảng đầu tiên — mũi tên này trúng mọi bong bóng có điểm bắt đầu <= vị trí đó. Lặp lại với bong bóng tiếp theo chưa bị trúng." },
    { name: "Partition Labels", difficulty: "Medium", note: "Chia chuỗi thành các đoạn sao cho mỗi ký tự chỉ xuất hiện trong đúng 1 đoạn, tối đa hóa số đoạn, dùng tham lam với vị trí xuất hiện cuối cùng.", hint: "Với mỗi ký tự, tìm vị trí xuất hiện CUỐI CÙNG của nó trong chuỗi. Duyệt qua chuỗi, mở rộng 'điểm kết thúc đoạn hiện tại' = max(điểm hiện tại, vị trí cuối cùng của ký tự đang xét). Khi vị trí duyệt chạm điểm kết thúc đó, đoạn hoàn tất." }
  ],
  walkthrough: {
